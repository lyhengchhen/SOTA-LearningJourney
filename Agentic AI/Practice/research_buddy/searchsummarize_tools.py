...
def search_arxiv(request: SearchRequest) -> FinalResponse:
    ...
    if ranking and ranking.rerank:
        results = reranker(request.query, results)
    ...
    return FinalResponse(
        ...
        query = request.query, 
        results = results,
        summary = generate_summary(request.query, SearchResult),
        response_metadata= ResponseMetadata(
...
def run_arxiv(request_dict: dict) -> dict: 
    request = SearchRequest.model_validate(request_dict)
    response = search_arxiv(request)
    return response.model_dump(mode="json") # Convert the FinalResponse back into a plain dict that the agent can read. 
